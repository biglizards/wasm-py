...
// forward declare
//static Type PyTuple_Type;

PyTupleObject* PyTuple_set_item_unchecked(PyObject* v, PyTupleObject* op, Py_ssize_t i) {
...
//#endif
    {
        ...
        op = PyObject_GC_NewVar(PyTupleObject, &PyTuple_Type, size);  // todo
        if (op == NULL)
            return NULL;
    }
    return op;
...
PyTuple_New(Py_ssize_t size) {
    ...
    // todo: why is it ok to remove this call? What does GC tracking mean?
//    _PyObject_GC_TRACK(op);
// todo 2: why does it start with refCount = 0?
    return (PyObject*) op;
}
...
{
    // ok so for now we're just assuming all things passed are integers
    // todo add slices
...
//};
//
PyMappingMethods tuple_as_mapping = {
    (lenfunc)tuplelength,
...
//    printf("OHBYTHEWAY, %ld\n", sizeof(PyTupleObject) - sizeof(PyObject *));
    printf("STRUCT: ");
    for (int i = 0; i < sizeof(Type)/sizeof(long); ++i) {
        printf("%08lX ", ((long*)ptr)[i]);
    }
    printf("\n");

//    printf("DEBUG_PRINT: %p, %p, %p, %p, %p\n",
//           &PyTuple_Type,
//           PyTuple_Type.tp_as_mapping,
//           &tuple_as_mapping,
//           &tuple_as_mapping.mp_subscript,
//           ((Type*)ptr)->tp_as_mapping
//           );
//    printf("DEBUG_PRINT2: %p, %p, %p %s\n",
//           PyTuple_Type.tp_as_number,
//           &number_methods,
//           (PyMappingMethods*)((Type*)ptr)->tp_as_number,
//           ((Type*)ptr)->tp_name
//    );
}
