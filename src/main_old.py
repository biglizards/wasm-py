...
def compile_instruction(i: dis.Instruction) -> Optional[Instruction]:
    if i.opname == 'LOAD_CONST':
        ...
        # todo wait no they arent -- it's only sometimes. In the general case, we jump to the end of the function
        # a good WASM JIT will transform this back into a ret instruction, so it's probably ok to do this all the time
        return None
    ...
    #         pass  # todo
    #     else:
    #         raise NotImplementedError('can only handle LEQ')
...
def main():
    ...
    # print(func_to_wasm(fib.__code__, 'fib'))
